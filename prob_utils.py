...
class GaussianLoss(Module):
    ...
    def forward(self, groundtruth, distr_params, mask=None, reduction="sum"):
        ...
        if mask is None:
            ...
            log_probs = distr.log_prob(groundtruth)[mask == 1]
            
        # print(log_probs.view(groundtruth.shape[0], -1).sum(-1))
        if reduction == "sum":
            return log_probs.view(groundtruth.shape[0], -1).sum(-1)
        elif reduction == "mean":
            return log_probs.view(groundtruth.shape[0], -1).mean(-1)
        elif reduction == "none":
            return log_probs
