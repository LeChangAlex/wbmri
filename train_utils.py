...
def get_validation_iwae(val_dataloader, mask_generator, batch_size,
                        model, num_samples, epoch, verbose=False, middle_mask=False):
    """
    ...
    with torch.no_grad():
        ...
        for batch in iterator:
            ...
            if next(model.parameters()).is_cuda:
                ...
                mask = mask.cuda()
            with torch.no_grad():
                iwae, rec_losses, kls, recs, masks = model.batch_iwae(batch, mask, num_samples, middle_mask=middle_mask)
                iwae = iwae[:init_size]
        ...
        return float(avg_iwae), rec_losses, kls, recs, masks, batch
