...
class MemoryLayer(nn.Module):
    ...
    def forward(self, input):
        # print(self.id, input.shape)
        if not self.output:
            # self.storage[self.id + str(input.device.index)] = input
            self.storage[self.id + str(input.device.index)] = input

            return input
        else:
            if self.id + str(input.device.index) not in self.storage:
            ...
            stored = self.storage[self.id + str(input.device.index)]
            if not self.add:
...
class ResBlock3d(nn.Module):
    ...
    def __init__(self, outer_dim, inner_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Conv3d(outer_dim, inner_dim, 1),
            nn.BatchNorm3d(inner_dim),
            nn.LeakyReLU(),
            nn.Conv3d(inner_dim, inner_dim, 3, 1, 1),
            nn.BatchNorm3d(inner_dim),
            nn.LeakyReLU(),
            nn.Conv3d(inner_dim, outer_dim, 1),
        )
